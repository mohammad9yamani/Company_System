...
<div class="container mt-5">
    ...
    <div id="step1" class="step active-step">
        <form id="transferForm" class="registration-container card p-4 shadow-sm">
            ...
            <button type="button" class="btn btn-primary btn-lg mt-3" onclick="fetchData(true)" id="confirmNextButton" disabled>Confirm and Next</button>
        </form>
    </div>

  <!-- Step 2: OTP Verification for Buyer, Vehicle, and Seller -->
<div id="step2" class="step" style="display: none;">
    <div class="card p-4 shadow-sm">
                ...
                <div class="card mb-3">
                    <div class="card-body text-center">
                        ...
                        <div id="buyerOtpSection" class="otp-section">
                            <button id = "buyerOtpButton"class="btn btn-primary otp-button" onclick="sendOTP('buyer')">Send OTP</button>
            ...
            <div class="col-md-4">
                <div class="card mb-3">
                    <div class="card-body text-center">
                        ...
                        <p id="vehicleData" class="card-text">
                        </p>
                    </div>
                </div>
            </div>
                ...
                <div class="card mb-3">
                    <div class="card-body text-center">
                        ...
                        <div id="sellerOtpSection" class="otp-section">
                            <button id="sellerOtpButton" class="btn btn-primary otp-button" onclick="sendOTP('seller')">Send OTP</button>
        ...
        <div class="button-group">
            ...
            <button type="button" class="btn btn-success btn-lg mt-3 step-button" onclick="ToThePrintDocument()" id="nextToContract" disabled>Next</button>
        </div>
    </div>
</div>
...
<style>
    .progress-container { display: flex; flex-direction: column; align-items: center; gap: 20px; }
    .steps { display: flex; width: 100%; align-items: center; justify-content: space-between; position: relative; max-width: 600px; }
    .circle { display: flex; align-items: center; justify-content: center; height: 40px; width: 40px; color: #999; font-size: 18px; font-weight: 500; border-radius: 50%; background: #fff; border: 3px solid #e0e0e0; }
    .circle.active { border-color: #4070f4; color: #4070f4; }
    .progress-bar { position: absolute; height: 4px; width: 100%; background: #e0e0e0; z-index: -1; }
    .indicator { position: absolute; height: 100%; width: 0%; background: #4070f4; transition: all 300ms ease; }
    .otp-section { max-width: 300px; margin: auto; }
    .otp-input, .otp-button, .btn-lg { padding: 10px; font-size: 16px; width: 100%; }
    .button-group { display: flex; justify-content: center; gap: 20px; }
    .step-button { width: 150px; }
    #step3 .btn-warning { width: 200px; margin: 0 auto; }
    #step4 .btn-success, #step4 .btn-secondary { width: 150px; margin: 0 10px; }
   /* Set each section to one-third width and ensure even spacing */
#step2 .col-md-4 {
    display: flex;
...
/* Make each card stretch within its column */
#step2 .card.mb-3 {
    flex: 1;
    display: flex;
    flex-direction: column;
...
/* Center-align text and elements within the card */
#step2 .card-body {
    display: flex;
    flex-direction: column;
    justify-content: center;
...
/* Additional styling to align buttons */
.otp-section {
    margin-top: 15px;
}
...
<script>
    ...
    function loadSessionData() {
        fetch('/load-session')
            ...
            .then(data => {
                ...
                if (data.step) nextStep(data.step);
                if (data.buyer_national_id) document.getElementById('buyer_national_id').value = data.buyer_national_id;
                if (data.seller_national_id) document.getElementById('seller_national_id').value = data.seller_national_id;
                if (data.vehicles_num) document.getElementById('vehicles_num').value = data.vehicles_num;
                if (data.vehicle_cost) document.getElementById('vehicle_cost').value = data.vehicle_cost;

                if (data.buyerVerified) {
                    document.getElementById('buyerVerified').style.display = 'block';
                    document.getElementById('buyerVerifySection').style.display = 'none';
                    document.getElementById('buyerOtpButton').style.display = 'none';

                }

                if (data.sellerVerified) {
                    document.getElementById('sellerVerified').style.display = 'block';
                    document.getElementById('sellerVerifySection').style.display = 'none';
                    document.getElementById('sellerOtpButton').style.display = 'none';
                } 

                document.getElementById("nextToContract").disabled = !(data.buyerVerified && data.sellerVerified);
    ...
    function validateFields() {
        ...
        confirmNextButton.disabled = !(buyerId && sellerId && vehicleNum && vehicleCost);
    }
    ...
    function ToThePrintDocument(){
    checkVerificationCompletion();
        nextStep(3);
    }

    function fetchData(MoveToNextStep) {
        saveFormData();
        const formData = new FormData(document.getElementById('transferForm'));
        formData.append('MoveToNextStep', MoveToNextStep);
        ...
        .then(data => {
            ...
            document.getElementById('vehicleData').innerHTML = `
            Vehicle Number: ${data.vehicleData.vehicle_number}<br>
            Make: ${data.vehicleData.make}<br>
            Model: ${data.vehicleData.model}<br>
    ...
    function checkVerificationCompletion() {
        fetch('/check-verification-completion')
            .then(response => response.json())
            .then(data => {
                if (data.buyerVerified && data.sellerVerified) {
    ...
    .then(data => {
        if (data.success) {
            ...
            console.error('Error creating transfer record:', data.message);
        }
    })
    .catch(error => console.error('Error creating transfer record:', error));
    }
